            ...
            {filtered.map((d) => (
              <div key={d.id} className="rounded-2xl border border-slate-100 bg-white p-4">
                ...
                <div className="mt-3 flex items-center justify-between text-xs text-slate-500">
                  ...
                  <span className="font-semibold text-slate-900">₹{d.consultation_fee} / consult</span>
                </div>

                <button
                  type="button"
                  onClick={() => bookConsultation(d)}
                  disabled={isBooked(d.id)}
                  className={`mt-3 flex w-full items-center justify-center gap-2 rounded-xl py-2 text-sm font-semibold ${
                    isBooked(d.id) ? "bg-teal-50 text-teal-600" : "bg-teal-600 text-white hover:bg-teal-700"
                  }`}
                >
                  <CalendarCheck size={15} /> {isBooked(d.id) ? "Requested" : "Book consultation"}
                </button>
              </div>
            ))}
            {filtered.length === 0 && (
